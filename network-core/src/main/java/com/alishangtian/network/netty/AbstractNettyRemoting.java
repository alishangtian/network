...
import com.alishangtian.network.common.*;
...
import com.alishangtian.network.processor.AsyncNettyRequestProcessor;
import com.alishangtian.network.processor.NettyRequestProcessor;
import com.alishangtian.network.protocol.RemotingSysResponseCode;
...
public abstract class AbstractNettyRemoting {
    ...
    protected void doBeforeRpcHooks(String addr, NetworkCommand request) {
        if (rpcHooks.size() > 0) {
    ...
    protected void doAfterRpcHooks(String addr, NetworkCommand request, NetworkCommand response) {
        if (rpcHooks.size() > 0) {
            for (RPCHook rpcHook : rpcHooks) {
     ...
     */
    public void processRequestCommand(final ChannelHandlerContext ctx, final NetworkCommand cmd) {
        ...
        if (pair != null) {
            ...
            try {
                pair.getObject2().submit(() -> {
                    try {
                        doBeforeRpcHooks(NetworkHelper.parseChannelRemoteAddr(ctx.channel()), cmd);
                        final RemotingResponseCallback callback = response -> {
                            doAfterRpcHooks(NetworkHelper.parseChannelRemoteAddr(ctx.channel()), cmd, response);
                            if (!cmd.isOnewayRPC()) {
                                if (response != null) {
                        ...
                        if (pair.getObject1() instanceof AsyncNettyRequestProcessor) {
                            ...
                            NetworkCommand response = processor.processRequest(ctx, cmd);
                            doAfterRpcHooks(NetworkHelper.parseChannelRemoteAddr(ctx.channel()), cmd, response);
                ...
                if ((System.currentTimeMillis() % 10000) == 0) {
                    log.warn(NetworkHelper.parseChannelRemoteAddr(ctx.channel())
            ...
            final NetworkCommand response = NetworkCommand.builder()
                    ...
                    .build().markResponseType();
            ctx.writeAndFlush(response);
            if (cmd.getCode() != RequestCode.CHANNEL_KEEP_ALIVE_PING_REQUEST) {
                log.error(NetworkHelper.parseChannelRemoteAddr(ctx.channel()) + error);
            }
        }
    }
     ...
     */
    public void processResponseCommand(ChannelHandlerContext ctx, NetworkCommand cmd) {
        final long opaque = cmd.getOpaque();
        ...
        if (responseFuture != null) {
            ...
            log.warn("receive response, but not matched any request, " + NetworkHelper.parseChannelRemoteAddr(ctx.channel()));
    ...
    public NetworkCommand invokeSyncImpl(final Channel channel, final NetworkCommand request,
        ...
        try {
            ...
            if (null == responseCommand) {
                if (responseFuture.isSendRequestOK()) {
                    throw new RemotingTimeoutException(NetworkHelper.parseSocketAddressAddr(addr), timeoutMillis,
                            responseFuture.getCause());
                } else {
                    throw new RemotingSendRequestException(NetworkHelper.parseSocketAddressAddr(addr), responseFuture.getCause());
                }
            }
    ...
    public void invokeAsyncImpl(final Channel channel, final NetworkCommand request, final long timeoutMillis,
        ...
        if (acquired) {
            ...
            try {
                channel.writeAndFlush(request).addListener((ChannelFutureListener) f -> {
                    ...
                    requestFail(opaque);
                    log.warn("send a request command to channel <{}> failed.", NetworkHelper.parseChannelRemoteAddr(channel));
                });
            } catch (Exception e) {
                responseFuture.release();
                log.warn("send a request command to channel <" + NetworkHelper.parseChannelRemoteAddr(channel) + "> Exception", e);
                throw new RemotingSendRequestException(NetworkHelper.parseChannelRemoteAddr(channel), e);
            }
        } else {
            if (timeoutMillis <= 0) {
    ...
    public void invokeOnewayImpl(final Channel channel, final NetworkCommand request, final long timeoutMillis)
            throws InterruptedException, RemotingTooMuchRequestException, RemotingTimeoutException, RemotingSendRequestException {
