...
public class NetworkHelper {
    ...
    public static NetworkCommand invokeSync(final String addr, final NetworkCommand request,
                                            final long timeoutMillis) throws InterruptedException, RemotingConnectException,
            RemotingSendRequestException, RemotingTimeoutException {
        long beginTime = System.currentTimeMillis();
        SocketAddress socketAddress = NetworkUtil.string2SocketAddress(addr);
        SocketChannel socketChannel = NetworkUtil.connect(socketAddress);
        if (socketChannel != null) {
            ...
            try {
                ...
                byteBufferBody.flip();
                return NetworkCommand.decode(byteBufferBody);
            } catch (IOException e) {
